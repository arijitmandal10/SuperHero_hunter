...
async function searchHeros(heroname) {
	...
	if (heroname.length > 0) {
		let url =
			...
			heroname +
			'&limit=8&ts=1&apikey=479af95fea303989d02e4a22757147e7&hash=b837dd71ea30a412d884f205f4da646f';
		...
		html = res
			.map((element) => {
                ...
                <div class="details">
                    ...
                    <div class="icon">
                        <i class="${
						favouritesCharacterIDs.has(`${element.id}`)
							? 'heart-icon fa-solid fa-heart'
...
function addToFavorites(c) {
	let heroInfo = {
		...
		comics: c.parentElement.parentElement.children[1].children[2]
			.innerHTML,
		series: c.parentElement.parentElement.children[1].children[3]
			.innerHTML,
		stories: c.parentElement.parentElement.children[1].children[4]
			.innerHTML,
